...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++){
        ...
        // about each game
        gameCard.innerHTML = `
        <img class="game-img" src="${games[i].img}" alt="${games[i].name}" style="max-width: 100%; height: auto; object-fit: contain;" />
        <h3>${games[i].name}</h3>
...
function filterUnfundedOnly() {
    ...
    let listGoalNotMet = GAMES_JSON.filter ( (game => game.pledged < game.goal));

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listGoalNotMet);
}
...
function filterFundedOnly() {
    ...
    let listGoalMet = GAMES_JSON.filter ( (game => game.pledged >= game.goal));

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(listGoalMet)
}
...
Currently, ${numUnfundedGames} game${numUnfundedGames === 1 ? '' : 's'} ${numUnfundedGames === 1 ? 'remains' : 'remain'} unfunded.
